...
public class FolderAdapter extends BaseAdapter {
	...
	public FolderAdapter(Context context) {
		...
		// mImageSize =
		// mContext.getResources().getDimensionPixelOffset(R.dimen.folder_cover_size);
	}
			...
			if (i == 0) {
				...
				if (mFolders.size() > 0) {
					...
					InitHelper.getImageLoader().display(
							"file://" + f.cover.path, holder.cover,
							ImageGridAdapter.getDisplayOptions());
				}
			} else {
	...
	class ViewHolder {
		...
		void bindData(Folder data) {
			...
			// .centerCrop()
			// .into(cover);
			InitHelper.getImageLoader().display("file://" + data.cover.path,
					cover, ImageGridAdapter.getDisplayOptions());
			// TODO 选择标识
		}
	}
